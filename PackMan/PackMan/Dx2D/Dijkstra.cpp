...
void Dijkstra::StartFindARoute(POINT start, POINT dest, vector<POINT>& moveVec)
{
	moveVec.clear();
	m_vecOpenNode.clear();
...
tagNode Dijkstra::GetMinCostNode()
{
...
void Dijkstra::UpdateEdgeNode(int dirX, int dirY)
{
	POINT pt = { m_stMinNode.ptMapIndex.x + dirX, m_stMinNode.ptMapIndex.y + dirY };

	// 이동할 수 있는 블럭인지 확인
	if (m_pMapTool->GetTileInfo(pt.x, pt.y).eTType != TERRAIN_TYPE::Unwalkable &&
		m_pMapTool->GetTileInfo(pt.x, pt.y).eOType == OBJECT_TYPE::None)
	{
...
void Dijkstra::MakeARoute(vector<POINT>& moveVec)
{
