...
void Camera::SetRenderScale(float width, float height)
{
	// 프로젝션 스케일링
	D3DXMatrixOrthoOffCenterLH(&m_stViewProjMatrix.Projection, 0, width, 0, height, -1, 1);
	D3DXMatrixTranspose(&m_stViewProjMatrix.Projection, &m_stViewProjMatrix.Projection);
	DeviceContext->UpdateSubresource(m_pViewProjBuffer, 0, 0, &m_stViewProjMatrix, 0, 0);

	//뷰포트 스케일링
	m_stViewPort.Width = width;
