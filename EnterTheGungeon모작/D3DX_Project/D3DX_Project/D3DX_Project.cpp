...
HWND		g_hWnd;								// 윈도우 핸들
POINT		g_ptMouse;
float		g_fTimeSpeed = 1.0f;

WCHAR		szTitle[MAX_LOADSTRING] = L"게임 지옥";    // 제목 표시줄 텍스트입니다.
MainGame* pMG;
POINT		ptWinSize;
	...
	_In_ int       nCmdShow)
{
	...
	// 매니져 해제
	g_pLoadManager->ReleaseAll();
	g_pLoadManager->ReleaseInstance();
	g_pSceneManager->ReleaseAll();
	...
	g_pTextManager->ReleaseInstance();
	g_pFileManager->ReleaseInstance();
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_D3DXPROJECT));
