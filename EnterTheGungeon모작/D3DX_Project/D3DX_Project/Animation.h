...
public:
	...
	void SetCurrFrame(T state, int curr);
	void SetElapsedTime(float f) { m_mapClip[m_eCurrState]->SetElapsedTime(f); }
	void SetCurrState(T state) { m_eCurrState = state; }
	...
	void SetConstantSizeScale(D3DXVECTOR2 v);
	...
	int GetCurrFrame() { return m_mapClip[m_eCurrState]->GetCurrFrame(); }
	int GetmaxFrame() { return m_mapClip[m_eCurrState]->GetMaxFrame(); }
	float GetElapsedTime() { return m_mapClip[m_eCurrState]->GetElapsedTime(); }
};
...
template<typename T>
inline void Animation<T>::SetCurrFrame(T state, int curr)
{
	m_mapClip[state]->SetCurrFrame(curr);
}
...
template<typename T>
inline void Animation<T>::SetConstantSizeScale(D3DXVECTOR2 v)
{
	for (auto p : m_mapClip)
		p.second->SetConstantScale(v.x, v.y);
}
